...
family_for_basename() {
  case "$1" in
    ...
    fm-brief.test.sh|fm-credential.test.sh|fm-vendor-auth-probe.test.sh|\
    ...
    fm-spawn-dispatch-profile.test.sh|fm-dispatch-restrict.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-install-herdr.sh|bin/fm-install-treehouse.sh|bin/fm-herdr-ci-cleanup.sh)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    bin/fm-lint.sh|bin/fm-install-shellcheck.sh|bin/fm-credential.sh|\
