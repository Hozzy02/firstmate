...
The only worker prompts outside that posture are one-time first-run directory-trust or hooks-review dialogs, which firstmate handles during the post-spawn check; there is no per-worker confirmation gate to enable auto-approve.
