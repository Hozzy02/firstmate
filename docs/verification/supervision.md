...
`tests/fm-session-lock-ancestry.test.sh` covers the portable side: unit cases drive both a reparented-shape fake process tree (a claude-matching ancestor whose own parent reports `ppid 1`) and a nested non-harness gap through `fm_session_lock_owned_by_self`, proving the former admits `CLAUDE_PID` while the latter refuses it, and three real-process e2e cases run the actual Stop auto-arm through a genuinely orphaned `bg-pty-host`/`bg-spare` chain - one where `CLAUDE_PID` names the live session and ownership is claimed, one with no `CLAUDE_PID` reproducing the original defect as inert rather than mis-owned, and one where `CLAUDE_PID` names a different live claude pid and ownership is still refused.
