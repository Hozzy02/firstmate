...
| `fm-dispatch-restrict.sh` | Set, list, or lift a durable per-home task dispatch restriction                      |
...
| `fm-credential.sh`       | Validate private credential policy and run one fixed provider operation with command-scoped delivery |
