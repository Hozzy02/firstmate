...
A beacon stale beyond grace is also healthy while the Stop-owned hand-off is pending, meaning `state/.claude-autoarm-epoch` records `rewake`, or `arming` with no live hook owner, younger than `FM_AUTOARM_HANDOFF_HOLD` (default 28800 seconds, the hook's declared timeout), because no watcher runs during the model turn the hook handed the wake to and the next Stop re-arms.
A failed outcome, an expired hand-off, or a live hook owner with a stale beacon still alarms.
...
`tests/fm-guard-stale-banner.test.sh` covers the pull-guard predicate, including the persistent-model fresh-leftover-beacon negative control, the auto-arm model's healthy fresh-beacon-without-a-watcher case, stale-beacon alarm, pending Stop-owned hand-off, and failed, expired, or wedged-arm alarms, and the extension model's live-watcher path, ownership-qualified fresh hand-off, held-lock failures, independently broken ownership signals, stale-beacon alarm, queued-wake warning, and Pi and pi-signed harness routing.
