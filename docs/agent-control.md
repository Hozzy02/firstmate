...
- On herdr, an opencode agent still registered as idle or done reads agent-free once its pane provably holds only a lone idle shell (the pane's own, or the worktree subshell `treehouse get` leaves under it), because opencode's hook-driven registration can outlive the exited TUI.
  A working or blocked agent, a live opencode in the foreground, and every other harness keep their registered verdict.
