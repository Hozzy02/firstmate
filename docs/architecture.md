...
For an ordinary crew whose last status is `paused:` or a durable `captain-held` transfer, or whose backlog item carries an active hold (`tasks-axi hold`), the normal-mode watcher surfaces at most one stale wake and then keeps an unchanged endpoint on that same cadence instead of wedge-escalating it; a backend-confirmed dead agent skips that initial wake.
...
If the newest same-branch run cannot bind to the crew's current code identity, older same-branch runs and terminal status-log history are not eligible as current state; the script header owns the exact fallback contract.
