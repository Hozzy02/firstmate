...
Claude ends a hook it no longer wants by signalling its process group, so the arm's `started ... (beacon fresh)` output followed by `Terminated: 15` and an `arm-interrupted` ledger row mean the harness stopped the hook, not that Firstmate replaced the cycle; the hook turns HUP, INT, and TERM into an ordinary exit that releases its owner lock and removes its captured output.
Lock steals never nest: a stale steal mutex is reclaimed in place, so a failed lock create returns instead of growing `.steal.steal...` names.
...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded steal-lock names across repeated steals and failed creates, bounded and successor-linked lifecycle rows, and a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination.
`tests/fm-subagent-pretool-check.test.sh` proves Claude retains only the non-status Bash seatbelts.
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, signalled-hook cleanup, stale and live session owners, unchanged AFK and need boundaries, single-flight, bounded failure retries, benign live-watcher cycle ends, one-notice failure episodes, and exit-2 translation.
